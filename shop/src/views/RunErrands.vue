<template>
  <div class=" v c " style="position: relative;margin-top: 10px;">
    ...
    <el-button style="position: absolute;right: 5%;top: 1%;width: 200px;height: 50px;" @click="dialogFormVisible = true"
      type="primary">AddShoop</el-button>
    ...
    <el-dialog title="" :visible.sync="dialogFormVisible" modal-append-to-body :close-on-click-modal="false">
      <el-form :model="form" ref="form" label-width="150px" :rules="rules">
        <h1 class="fs-large padding-2-0x">Add Run Service</h1>
        ...
        <el-form-item label="deliveryTel" prop="deliveryTel">
          <el-input v-model.number="form.deliveryTel" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item label="deliveryPerson" prop="deliveryPerson">
          <el-input v-model="form.deliveryPerson" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item label="price" prop="price">
          <el-input-number v-model="form.price" :precision="2" :step="0.1" :max="9999999" :min="0"
...
<script>
import { addDelivery, deliveryList } from '@/api/RunErrands'
  ...
  methods: {
    ...
    toAddDat() {
      this.$refs['form'].validate((valid) => {
        if (valid) {
          this.loading = true
          addDelivery(this.form).then(res => {
